...
import { startMockServer, type RunningMockServer } from "../../src/mock/server";
...
describe("API and protocol mocks", () => {
  ...
  it("supports websocket echoes", async () => {
    const payload = await new Promise<string>((resolve, reject) => {
      const socket = new WebSocket("ws://127.0.0.1:3201/ws/events");

      socket.once("message", () => {
        socket.send("hello-broker");
      });

      socket.once("error", reject);
      socket.on("message", (message) => {
        const data = JSON.parse(message.toString()) as { type: string; payload?: string };
        if (data.type === "echo") {
          socket.close();
    ...
    expect(payload).toBe("hello-broker");
  });
});

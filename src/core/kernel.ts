...
import { AgenticAdvisor, RuntimeConfiguration, TestExecutionResult, UiDriver } from "./contracts";
...
export class AutomationKernel {
  constructor(private readonly registry = createDefaultRegistry()) {}
  ...
  async bootstrap(options: BootstrapOptions = {}): Promise<ExecutionContext> {
    ...
    const logSinks = config.adapters.logSinks.map((sinkRef) => ({
      sink: this.registry.createLogSink(sinkRef.name, sinkRef.options),
      options: sinkRef.options
    }));

    const logger = new CompositeLogger(logSinks);
  ...
  async publishResult(
    ...
    await Promise.all(
      context.config.adapters.resultSinks.map(async (sinkRef) => {
        const sink = this.registry.createResultSink(sinkRef.name, sinkRef.options);
        await sink.publish(result, sinkRef.options);
      })
    );
  }

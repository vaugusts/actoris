...
export function createDefaultKeywordRegistry(): KeywordRegistry {
  return new KeywordRegistry()
    ...
    .register("loginWithData", async (context, state) => {
      ...
      const username = String(state.data.username);
    ...
    .register("assertDashboardGreeting", async (context, state, args) => {
      ...
      const dashboardPage = new DashboardPage(driver, context.config.project.baseUrl);
      const expected = String(
        resolveTemplate(args?.expected ?? "", {
          data: state.data,
          vars: state.variables
      ...
      const actual = await dashboardPage.greeting();
      if (actual !== expected) {
...
function stateful(context: ExecutionContext): Record<string, any> {
  const key = "__keywordInternals";
  const bag = (context.data[key] ?? {}) as Record<string, any>;
